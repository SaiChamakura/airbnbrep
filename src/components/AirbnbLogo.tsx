...
/**
 * Airbnb Logo Component
 * Prioritizes /logo.png uploaded by user with graceful fallback to official vector SVG.
 */
export const AirbnbLogo: React.FC<AirbnbLogoProps> = ({
  className = 'w-8 h-8',
  imageSrc,
}) => {
  const candidateSources = imageSrc
    ? [imageSrc]
    : ['/logo.png', '/airbnb-logo.png', '/airbnb-logo.svg'];
  ...
  if (!allFailed && candidateSources.length > 0) {
    return (
      <img
